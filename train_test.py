...
def train(classifier, opti, label_list, batch_size):
    opti.zero_grad()
    loss = 0
...
def train_times(classifier, opti, label_list, batch_size = 256, repeat = 1):
    classifier.train()
    for _ in range(repeat):
        train(classifier, opti, label_list, batch_size)
    
def get_correct(classifier, x, y, correct, total):
...
def test(classifier, label_list):
    classifier.eval()
    correct, total = 0, 0
    for label in label_list:
        x = test_groups[label]["x"]
        y = test_groups[label]["y"]
        correct, total = get_correct(classifier, x, y, correct, total)
    return(correct / total)

def session(classifier, label_list, target = .9, length = 500):
    opti = Adam(classifier.parameters())
    correct_list = []
    for i in tqdm(range(length)):
        train_times(classifier, opti, label_list)
        correct_list.append(test(classifier, label_list))
...
def get_image_list(classifier, actual, predicted):
    classifier.eval()
